...
Application images require the build manifest generated by `docker.sh build`.
For the harvester, the build also verifies that the nested
`lareferencia-dark-lib` JAR has the same SHA-256 as the reactor output; this
prevents packaging a stale library from `target/`.
