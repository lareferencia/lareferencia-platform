...
1. Access the project's root directory:
   ...
   ```
2. Start the interactive Docker wizard:
   ```bash
...
4. The wizard will prompt for:
   ...
   - **Cron Schedule**: You can enable a daily backup that runs autonomously overnight. When `crontab` is not available, the wizard installs a **systemd timer** instead.
